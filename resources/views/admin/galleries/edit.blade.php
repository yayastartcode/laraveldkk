...
<div class="max-w-4xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-yellow-400">Edit Gallery Item</h1>
    ...
    <form action="{{ route('admin.galleries.update', $gallery) }}" 
        ...
        <div>
            ...
            <textarea name="description" 
                      rows="4" 
                      class="w-full px-4 py-2 rounded bg-zinc-900 text-yellow-100 border border-yellow-400 focus:outline-none focus:ring-2 focus:ring-yellow-400">{{ old('description', $gallery->description) }}</textarea>
            @error('description')
                <p class="mt-1 text-red-500 text-sm">{{ $message }}</p>
            @enderror
        </div>
